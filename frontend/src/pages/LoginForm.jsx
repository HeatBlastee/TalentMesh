...
import DummyLoginModal from "../components/DummyLoginModal";
...
const LoginForm = () => {
   ...
   const redirectByRole = async (role, user) => {
      updateUser(user);
      if (role === "admin") {
   ...
   const onSubmit = async (values) => {
      ...
      try {
         ...
         if (response?.status === "success") {
            await redirectByRole(response.user?.role, response.user);
         } else {
            setErrorMessage(
   ...
   // ── Dummy login ────────────────────────────────────────────
   const handleDummyLogin = async (credentials, role) => {
      setLoadingRole(role);
      setErrorMessage("");
      try {
         ...
         if (response?.status === "success") {
            ...
            setShowDummy(false);
            setErrorMessage(
               response?.data?.detail || "Dummy login failed. Please try again.",
            );
         }
      } catch {
         setShowDummy(false);
   ...
   return (
      <>
               ...
               <div className="w-full max-w-md">
                  ...
                  >
                     ...
                     <button
                        ...
                        disabled={isSubmitting}
                        className="w-full py-2.5 rounded-xl bg-white hover:bg-slate-50 text-slate-600 text-sm font-medium border border-slate-300 hover:border-indigo-400 hover:text-indigo-600 transition-all cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed"
                     >
                        🧪 Try Dummy Login
                     </button>
         ...
         {showDummy && (
            <DummyLoginModal
               onClose={() => setShowDummy(false)}
               onLogin={handleDummyLogin}
               isSubmitting={loadingRole !== null}
